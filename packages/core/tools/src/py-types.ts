...
\`run_code\` takes two required arguments: \`code\` — the body of an async Python function (top-level \`await\` and \`return\` both work) — and \`description\`, a short summary of what the program does. At run time exactly two of the names declared below are bound: \`tools\` and \`ToolCallError\`; everything else is a STATIC STUB describing argument and return types — the \`TypedDict\` classes do NOT exist at run time, so build arguments as plain \`dict\`/\`list\` JSON values: \`await tools.name({"field": 1})\`, never \`FooArgs(field=1)\`, which raises \`NameError\`). Inside the program:

- Call tools as \`await tools.name(args)\` — subscript for exotic, reserved, or underscore-leading names: \`await tools["my-tool"](args)\`. Every call resolves to the tool's typed canonical JSON value (each method's return type below). Tool arguments must be lossless JSON.
- A failed call raises \`ToolCallError\` — \`.toolName\` names the tool, the message is human-readable; wrap in \`try/except\` to handle and continue.
- Independent read-only calls MAY overlap under \`asyncio.gather\` (safe calls run concurrently; mutating calls run alone, in submission order). Sequence dependent work with \`await\`.
- Emit the answer with \`print(...)\` and/or a top-level \`return <value>\` (lossless JSON) — only what you print or return is program output. An image-bearing tool result is attached for you to inspect on the next step; other intermediate results stay out of the conversation — extract just what you need.
