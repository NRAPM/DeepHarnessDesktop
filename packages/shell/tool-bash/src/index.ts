...
function bashDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a bash command (`bash -c`) and return its stdout/stderr. '
    + 'Each call runs in a fresh shell: no state (cwd, variables, functions) persists — '
    + 'pass `workdir` instead of using `cd`. Non-zero exits are reported as `[exit code: N]`. '
    + `Harness environment facts are exposed through managed \`$${DSH_ENV_PREFIX}*\` variables; inspect them when needed. `
  ...
  return base + ' A command the sandbox may deny is safe to attempt: run it and read the marker rather '
    + 'than assuming the denial. When denied and a wider mode would let it succeed, escalate immediately '
    + 'in the same turn — the one sanctioned exception to a denial: retry the exact same command once with '
    + '`sandbox_permissions` (the narrowest wider mode that suffices) plus a one-sentence `justification`. '
    + 'Do not detour through chat to ask permission first — the approval prompt raised by that retry is '
    + 'how the user consents. If approval prompts are disabled, there is no exception: a denial is final — '
    + 'do not set `sandbox_permissions`. Never escalate speculatively: ground the request in a real denial — '
    + 'normally the one this command just hit; escalating up front is fine only when this session already '
    + 'denied the same access. A rejected escalation is final for that command — stop and explain, never '
    + 'work around it — but it does not forbid attempting or escalating other commands later.'
}
